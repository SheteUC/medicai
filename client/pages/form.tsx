...
class MyForm extends Component {
  ...
  handleInputChange = (event) => {
    ...
    // Check if the input element is a checkbox and handle its value accordingly
  ...
  handleSubmit = (event) => {
    ...
    const { name, ethnicity, gender, age, additionalComments, conditions, patientcond } = this.state;
  ...
  render() {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh'}}>
        <form className="max-w-md p-4 bg-white rounded-lg shadow-lg" onSubmit={this.handleSubmit} style={{ width: 400 }}>
          <h1 style={{ textAlign: 'center', color: 'black' }}>Patient Search Form</h1>
          <div className="mb-4">
          ...
          <div className="mb-4">
            ...
            <textarea
               ...
               onChange={this.handleInputChange}
               className="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-indigo-500"
               style={{ resize: 'none', height: '200px' }} // Add this style to disable resizing
               
            ></textarea>
          </div>
          <div className="mb-4">
            <label htmlFor="patientcond" className="block text-sm font-medium text-gray-700">Patient Condition:</label>
            <textarea
               id="patientcond"
               name="patientcond"
          ...
          <button type="submit" className="w-full px-4 py-2 text-white bg-indigo-500 rounded-md hover-bg-indigo-600 focus:ring focus:ring-indigo-300">
            Submit
          </button>
        </form>
      </div>
